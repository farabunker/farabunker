...
- **Tiers.** Orchestration runs on the most capable tier that executes. Implementers default
  to the middle tier, mechanical edits to the cheapest (`AGENTS.md`, "Subagent-driven
  development").
- **The top tier plans; it never executes.** Keyed to the tier, not the role: dispatched as a
  subagent it returns a plan or analysis to its caller, never a file edit and never the work
  itself.
