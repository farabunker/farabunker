...
| `FARABUNKER_QUEUE_LOG_LEVEL` | `INFO` | The level `models.queue` and `models.contracts.engines` log at — the execution queue's eviction/unload/precautionary-call lines (`docs/OPERATIONS.md` "The execution queue's memory governance"). An unrecognized value falls back to `INFO` rather than failing to boot. |
