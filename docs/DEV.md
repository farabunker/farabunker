...
| `FARABUNKER_QUEUE_LOG_LEVEL` | `INFO` | The level `models.queue`, `models.contracts.engines`, and `models.registry` log at — the execution queue's eviction/unload/precautionary-call lines and the registry's footprint-provenance refusals (`docs/OPERATIONS.md` "The execution queue's memory governance"). An unrecognized value falls back to `INFO` rather than failing to boot. |
...
full [--max-others N] [touched-modules...]` runs the named runs (module order under two flag
states, the touched-module scope when given, and the posture sweep -- eight runs with touched
modules, six without) plus `makemigrations --check --dry-run` and `check`, logging each to
