...
| `jobs.py` | The `agent.turn` job kind's other three dotted paths: `plan_turn` (the admission snapshot), `summarize_turn` (a one-line `/queue/` row, touches no database), and `on_turn_terminal` (fixes up a stranded placeholder turn). |
